    ...
    : describe("Lottery", async () => {
          ...
          describe("constructor", async () => {
              it("contract created as expected", async () => {
                  ...
                  assert(
                      playersLength.toString() === "0",
                      "Players data not correct"
                  )
                  assert(
                      interval.toString() ===
                          networkConfig[chainId].lottery.interval,
          ...
          describe("enter value to play", async () => {
              it("one enter - one player", async () => {
                  ...
                  const lotteryBalance = await ethers.provider.getBalance(
                      lottery.address
                  )

                  assert(
                      playerAddress === signer.address,
                      "Created wrong player"
                  )
                  assert(
                  ...
                  assert(
                      playersCount.toString() === "1",
                      "Wrong players calculation"
                  )
              })
